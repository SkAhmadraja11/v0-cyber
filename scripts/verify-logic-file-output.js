...
const sources = [
    ...
    { name: "VirusTotal", detected: true, confidence: 45, reason: "Heuristic Match" },
    { name: "WHOIS / DNS", detected: false, confidence: 0 }
];

const score = calculateRiskScore(sources);
let resultText = `Risk Score: ${score}\n`;

if (score >= 85) {
    resultText += "SUCCESS: Score is High (>= 85)";
} else {
    resultText += "FAILURE: Score is too low";
}

fs.writeFileSync('verification_result.txt', resultText);
